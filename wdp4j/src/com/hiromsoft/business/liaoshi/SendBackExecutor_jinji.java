...
public class SendBackExecutor_jinji extends BaseBusinessExecutor {
	...
	public void execute(Session dbsession, UserDataForm userDataForm,
		...
		try{
			pst1=dbsession.connection().prepareStatement("select * from  designer_userform162 where (formstate='2') and id=?");
			
			
			pst=dbsession.connection().prepareStatement("update designer_userform162 set formstate=1 where id=?");
			...
			for(int num=0;num<rowidx;num++){
				...
				if(rst.next())
					ok=true;
				rst.close();
				if(!ok) throw new Exception("已经完成审核的申报表不能重新填写");
