...
public class SendBackExecutor_pingshen extends BaseBusinessExecutor {
	...
	public void execute(Session dbsession, UserDataForm userDataForm,
		...
		try{
			pst1=dbsession.connection().prepareStatement("select * from  designer_userform159 where (formstate='2') and id=?");
			
			
			pst=dbsession.connection().prepareStatement("update designer_userform159 set formstate=1 where id=?");
