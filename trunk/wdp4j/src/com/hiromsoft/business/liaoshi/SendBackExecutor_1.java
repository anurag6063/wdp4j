...
public class SendBackExecutor_1 extends BaseBusinessExecutor {
...
//		最多复制100行
		...
		try{
			pst1=dbsession.connection().prepareStatement("select * from  designer_userform132 where (formstate='2') and id=?");
			
			
			pst=dbsession.connection().prepareStatement("update designer_userform132 set formstate=1 where id=?");
