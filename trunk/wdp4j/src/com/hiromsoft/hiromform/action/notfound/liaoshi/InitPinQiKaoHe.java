...
public class InitPinQiKaoHe extends BaseOnNotFoundAction{
	...
	public void execute(HttpServletRequest request,
		...
		aaa.put("ID", "0");
		aaa.put("jsxm", user.getDisplayName());  //姓名
		aaa.put("field0", user.getDepartment()); //部门
		...
		try{
			pst=dbsession.connection().prepareStatement("select field4 from designer_userform139 where yhbh=? and khbh=?");
		...
		aaa.put("field21", user1.get("CJGZSJ")); //考核期限开始
		aaa.put("field22", user1.get("CJGZSJ2"));//考核期限结束
		aaa.put("yqkh", user1.get("yq"));//是否延期考核
		
		if(firstDeptAsDefault){
			aaa.put("field1", user1.get("field0"));   //岗位类别
			aaa.put("field2", user1.get("field1"));   //岗位级别
			aaa.put("field101", user1.get("field15"));   //岗位
		}else{
			aaa.put("field0", user1.get("field14")); //部门
			aaa.put("field1", user1.get("field18"));   //岗位类别
			aaa.put("field2", user1.get("field19"));   //岗位级别
			aaa.put("field101", user1.get("ZW"));   //岗位
		}
		
		dbsession.saveOrUpdate((String)"designer_userform137",aaa);
		dbsession.flush();
