...
public class InitPinQiKaoHe_jiaoshi extends BaseOnNotFoundAction{
	...
	public void execute(HttpServletRequest request,
		...
		try{
			pst=dbsession.connection().prepareStatement("select field6 from designer_userform139 where yhbh=? and khbh=?");
		...
		aaa.put("field19", user1.get("XB"));   //性别
		...
		if(firstDeptAsDefault){
			...
			aaa.put("field20", user1.get("ZW"));   //行政职务
		}else{
			aaa.put("field0", user1.get("field14")); //部门
			aaa.put("field1", user1.get("field18"));   //岗位类别
			aaa.put("field2", user1.get("field19"));   //岗位级别
			aaa.put("field20", user1.get("ZW"));   //岗位
		}
		
		dbsession.saveOrUpdate((String)"designer_userform136",aaa);
		dbsession.flush();
